...
export default function EditHabitPage() {
  ...
  const [habitToEdit, setHabitToEdit] = useState<Habit | undefined | null>(
    undefined
  ); // undefined for loading, null for not found
  ...
  if (habitToEdit === undefined) {
    return (
      <div
        className="flex flex-col items-center justify-center min-h-[calc(100svh-10rem)] text-center p-4"
        lang="fa"
      >
        <Loader2 className="h-12 w-12 animate-spin text-primary mb-4" />
        <p className="text-muted-foreground">
          در حال بارگذاری اطلاعات عادت برای ویرایش...
        </p>
      </div>
    );
  }
